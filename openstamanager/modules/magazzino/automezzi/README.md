---
description: Come gestire gli Automezzi con OpenSTAManager
icon: circle
metaLinks:
  alternates:
    - >-
      https://app.gitbook.com/s/imnORsInuaaXiiRnlr6m/openstamanager/modules/magazzino/giacenze-sedi
---

# Automezzi

{% hint style="info" %}
Il modulo **Automezzi** permette all’azienda di gestire le informazioni riguardanti gli automezzi presenti in OpenSTAManager.
{% endhint %}

<figure><img src="../../../../.gitbook/assets/image (920).png" alt=""><figcaption></figcaption></figure>

### Widget

* Stampa giacenza&#x20;
* Stampa carico odierno

<figure><img src="../../../../.gitbook/assets/image (921).png" alt=""><figcaption></figcaption></figure>

## Creazione

Per creare un nuovo Automezzo si dovrà cliccare sul tasto (+).

Andranno qui inserite le informazioni relative al nuovo automezzo:

* Nome
* Targa&#x20;

<figure><img src="../../../../.gitbook/assets/image (922).png" alt=""><figcaption></figcaption></figure>

## Modifica

Cliccando sul record da modificare si aprirà la schermata di dettaglio, in cui si troveranno diverse sezioni:

### Automezzo

* Nome
* Targa
* Descrizione

<figure><img src="../../../../.gitbook/assets/image (923).png" alt=""><figcaption></figcaption></figure>

### Utenti responsabili automezzo

<figure><img src="../../../../.gitbook/assets/image (924).png" alt=""><figcaption></figcaption></figure>

{% hint style="info" %}
E' possibile aggiungere un nuovo responsabile assegnando l'automezzo nel campo **Sede** all'interno del gruppo utenti desiderato in **Strumenti/Gestione accessi/Utenti e permessi** e aggiornare i dati dell'utente che si vuole nominare responsabile.
{% endhint %}

{% hint style="info" %}
Si possono inserire più responsabili appartenenti anche ad un diverso gruppo di utenti: Amministratori, Agenti, Clienti e Tecnici.
{% endhint %}

### Magazzino automezzo

<figure><img src="../../../../.gitbook/assets/image (925).png" alt=""><figcaption></figcaption></figure>

{% hint style="info" %}
Qui è possibile monitorare e gestire gli articoli di magazzino che si trovano all'interno di ciascun automezzo.
{% endhint %}

### Registro di viaggio

Questa sezione consente di monitorare l'utilizzo operativo dei veicoli aziendali, tracciando gli spostamenti, i chilometri percorsi e i costi associati a ciascuna uscita.

<figure><img src="../../../../.gitbook/assets/image (926).png" alt=""><figcaption></figcaption></figure>

{% hint style="warning" %}
Per poter assegnare un Tecnico ad un viaggio, deve prima essere stato assegnato all'automezzo nella sezione **Utenti responsabili automezzo**.&#x20;
{% endhint %}

E' inoltre possibile firmare il registro di viaggio tramite il pulsante disponibile sotto la voce **Firma**.

<figure><img src="../../../../.gitbook/assets/image (927).png" alt=""><figcaption></figcaption></figure>

## Plugin

Selezionando uno specifico record si può accedere a diversi plugin nella barra laterale della pagina:

* Manutenzioni
* Scadenze
* Danni

## Informazioni aggiuntive

{% content-ref url="/broken/pages/72Yfrc1Lwae1uKbvMzWl" %}
[Broken link](/broken/pages/72Yfrc1Lwae1uKbvMzWl)
{% endcontent-ref %}
